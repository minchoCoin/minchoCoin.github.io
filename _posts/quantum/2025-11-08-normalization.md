...
Born의 통계적 해석에 의하면 파동함수의 제곱은 해당 시간 해당 위치에서 입자를 발견할 확률이다. 이때 파동함수의 제곱이란 $\vert\Psi(x,t)\vert^2$ 이며, 이는 $\Psi ^* \Psi$ 를 의미하고 $ \Psi^* $ 는 $ \Psi $ 의 복소켤레이다.

$(a+bi)*(a-bi) = a^2+b^2$ 이므로 $\vert\Psi(x,t)\vert^2 >0$ 이다.

# Normalization
일반적으로 확률의 합은 1이다.
$$ \int_{-\inf}^{\inf} \vert\Psi(x,t)\vert^2 dx =1 $$
