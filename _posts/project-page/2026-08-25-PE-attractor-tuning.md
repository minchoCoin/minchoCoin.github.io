...
- Starting from the backbone proposal $\tilde{y}_0$, it repeatedly refines the output embedding
  ...
  - $\tilde{y}_{t+1} = T_{\theta_a}(\tilde{y}_t, \tilde{y}_0), \quad \text{where } \tilde{y}_0 = T_{\theta_b}(x)$
...
- $\mathcal{A}_{\theta_a}(\tilde{y}^{\star};\tilde{y}_0)=T_{\theta_a}(\tilde{y}^{\star},\tilde{y}_0)-\tilde{y}^{\star}=0 \Rightarrow \tilde{y}^{\star}=\operatorname{RootFind}(\mathcal{A}_{\theta_a}(\cdot;\tilde{y}_0);y_0)$
