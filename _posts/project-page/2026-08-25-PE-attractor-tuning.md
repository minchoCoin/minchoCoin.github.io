...
- Rather than rolling out recurrent steps to reach a fixed point, attractor solve for the convergence
  - $\mathcal{A}_{\theta_a}(\tilde{y}^{\star};\tilde{y}_0) := T_{\theta_a}(\tilde{y}^{\star},\tilde{y}_0) - \tilde{y}^{\star} = 0
    \Rightarrow
    \tilde{y}^{\star} = \mathrm{RootFind}(\mathcal{A}_{\theta_a}(\cdot;\tilde{y}_0); y_0)$
