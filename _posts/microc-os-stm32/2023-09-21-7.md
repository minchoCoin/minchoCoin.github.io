...
task가 OSTimeDly()를 실행하면, task는 더이상 실행 대상이 아니기 때문에 μC/OS-III의 ready list에서 제거됩니다(141 페이지의 "The Ready List" 장에서 설명됨). 또한, μC/OS-III는 다음으로 중요한 ready 상태의 task를 실행해야 하므로 스케줄러가 호출된다.
# Reference
