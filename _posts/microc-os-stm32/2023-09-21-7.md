...
ISR이 μC/OS-III에 의해 제공된 "post" 함수 중 하나를 호출하면, 게시된 데이터의 복사본과 원하는 목적지가 "holding" 큐에 배치된다. 모든 중첩된 ISR이 완료되면, μC/OS-III는 ISR handler task(OS_IntQTask())로 context switch를 수행하며, ISR handler task는 holding 큐에 배치된 정보를 적절한 task에게 다시 post한다. 이 추가 단계는 wait list에서 task를 제거하고, ready list에 넣는 등의 시간을 소모하는 동작을 수행하는데 필요할 수 있는 interrupt disable의 시간을 줄이기 위해 수행된다.
