...
- m is the clipping value determined by the min-max method, $m=maximum(|R|)$
...
- $A=S_A\cdot I_A$, $I_A=\lfloor \frac{A'}{S_A} \rceil=\lfloor \frac{S_Q\cdot S_K}{S_A}\cdot (I_Q\times I_K^T) \rceil$, where $S_A=\frac{2m}{2^k-1}, m=maximum(|A'|)$
